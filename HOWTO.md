...
- **Tools ▾** — the occasional ones: **✓ Validate page**, **🔍 OCR this page (vision)**, **⇄ Swap test/key**, **▼ Collapse all**, **▲ Expand all**, **🔗 Group questions**.
...
2. **Link the sub-questions to it.** Either pick the context from each question card's Context dropdown one at a time, or select several question cards (or their bounding boxes on the PDF — **Tools ▾ → 🔗 Group questions** to enter selection mode) and use **Assign to context ▾** in the sticky selection bar to link them all in one step.
