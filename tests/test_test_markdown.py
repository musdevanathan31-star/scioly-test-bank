"""
Coverage for assessments.py's markdown rendering — the printable "Assessment" and
...
def test_the_test_copy_never_contains_an_answer():
    ...
    out = assessments.render_questions_markdown(snap, title="T", answers="none")
...
def test_the_key_contains_every_answer():
    ...
    out = assessments.render_questions_markdown(snap, title="T", answers="section")
...
def test_inline_layout_puts_each_answer_under_its_own_question():
    ...
    out = assessments.render_questions_markdown(snap, title="T", answers="inline")
...
def test_points_are_pluralised_and_totalled():
    out = assessments.render_questions_markdown([mcq(points=1), frq(points=3)],
...
def test_free_response_leaves_writing_room_on_the_paper_copy():
    paper = assessments.render_questions_markdown([frq()], title="T", answers="none")
    assert "\n" * assessments._FRQ_ANSWER_LINES in paper
...
def test_inline_layout_replaces_writing_room_with_the_answer():
    ...
    inline = assessments.render_questions_markdown([frq(answer="Because Ohm.")],
                                               title="T", answers="inline")
    assert "Because Ohm." in inline
    assert "\n" * assessments._FRQ_ANSWER_LINES not in inline
...
def test_images_are_named_since_markdown_cannot_carry_them():
    ...
    out = assessments.render_questions_markdown([q], title="T", answers="none")
...
def test_a_shared_passage_prints_once_above_its_first_question():
    ...
    for q in (q1, q2):
        q["context_id"] = "c1"
        q["_context"] = ctx
    out = assessments.render_questions_markdown([q1, q2], title="T", answers="none")
...
def test_title_and_subtitle_are_rendered():
    out = assessments.render_questions_markdown([mcq()], title="Circuit Lab",
...
def test_unknown_layout_is_refused_rather_than_silently_omitting_answers():
    ...
    with pytest.raises(ValueError, match="unknown answers layout"):
        assessments.render_questions_markdown([mcq()], title="T", answers="keys")


def test_missing_key_is_stated_rather_than_left_blank():
    q = mcq(answer="")
    out = assessments.render_questions_markdown([q], title="T", answers="section")
    assert "(no key recorded)" in out


def test_empty_snapshot_still_renders_a_valid_document():
    out = assessments.render_questions_markdown([], title="Empty", answers="section")
    assert out.startswith("# Empty")
    assert "0 questions" in out


# ---------------------------------------------------------------------------
# Reuse detection for the Prepare-test pool (assessments.used_question_keys)
# ---------------------------------------------------------------------------
...
def test_used_keys_span_kept_and_published_and_skip_the_current_test(tmp_path,
    ...
    import seasons as seasons_mod, events as events_mod, assessments as t
    ...
    t1 = t.get_assessment_for(w1.window_id, slug)
    t2 = t.get_assessment_for(w2.window_id, slug)

    t.update_assessment_kept(t1.assessment_id, [{"bucket": "b", "number": "5", "max_points": 1}])
    t.update_assessment_kept(t2.assessment_id, [{"bucket": "b", "number": "9", "max_points": 1}])

    # Building t2: q5 is used (by t1), q9 is t2's own and must not be hidden
    # from the person currently choosing it.
    used = t.used_question_keys("2027", exclude_assessment_id=t2.assessment_id)
    assert "b::5" in used
