...
| `GET /event/<slug>/api/pdf/<pdf>/outline` | PyMuPDF-extracted PDF outline (`doc.get_toc()`) for the review-page left drawer (`#outlineDrawer`, opened by `☰`), which since Phase 2b (2026-08) also holds page thumbnails unconditionally — the frontend hides the outline section entirely when this returns empty, which is most test PDFs (13 of 14 sampled in this repo have no TOC). Returns `[{level, title, page}]`. |
