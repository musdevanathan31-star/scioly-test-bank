...
**Why `loadtest_students.py` measures rather than computes "how many students can test at once"**: the `--threads`/lock constraints above bound *where* concurrency can come from, but not how much of it the answer-save path can actually absorb — that depends on `testing.py`'s `RESPONSES_FILE` (§7): every `save_answer()` call read-parses the *entire* file (every response for every test, ever — not scoped per-test), merges one answer in, and rewrites the entire file, inside one `threading.RLock` shared across every student on every test. That cost scales with accumulated response-file size and write frequency, neither of which shows up in a CPU/RAM spec sheet, so the only trustworthy number comes from generating real concurrent load against that real endpoint and watching where latency/error rate breaks down. The script talks HTTP to a real running instance rather than importing `review_app.py` in-process for the same reason `--workers`/`--threads` matter at all here: in-process calls never go through gunicorn's actual worker/thread scheduling, so they couldn't exercise the thing being measured. It deliberately does not script the throwaway season/window/test creation itself (HOWTO.md's walkthrough has that as a manual first step) — that chain has no HTTP delete route if it failed halfway, so a human doing it once through the UI they already trust is lower-risk than a new automated chain against a production server.
