...
**Why `loadtest_students.py` measures rather than computes "how many students can test at once"**: the `--threads`/lock constraints above bound *where* concurrency can come from, but not how much of it the answer-save path can actually absorb, and that second question was never computable from a CPU/RAM spec sheet — it had to be measured against the real endpoint. A first production run (`--steps 5,10,20,40,80,160`) proved this concretely: p50 latency grew super-linearly with concurrent students (40→0.14s, 80→0.65s, 160→2.36s — roughly ×3.6 per doubling, not ×2), even with zero HTTP errors, because every `save_answer()` call was read-parsing, merging into, and rewriting `assessments.py`'s single `RESPONSES_FILE` — every response for every test, ever, not scoped per-test — inside one `threading.RLock` shared across every student on every test. That's fixed now: `assessments.py` stores one file per `(test_id, username)` pair under `RESPONSES_DIR` (see §7 and the module's own docstring), so concurrent saves from different students, or the same student on different tests, acquire different locks and touch different (small) files entirely — the cost of one save no longer scales with how much *other* response data exists anywhere in the app.
...
- **`assessments.py`** — `AssessmentWindow` (a scheduled open/close span, possibly spanning several days, covering one or more events for one season), `Test` (one event's test within one window), and `Response` (one student's answers/grading for one test). Three separate files (`assessment_windows.json`, `assessments.json`, `test_responses.json`) rather than one combined file, so a burst of concurrent student autosave traffic against `test_responses.json` (by far the highest-write-volume of the three) never contends with a coach loading the dashboard.
