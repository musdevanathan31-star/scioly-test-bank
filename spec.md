...
**Navigation** is consolidated into one ☰ navicon menu in `templates/_user_badge.html` — the single fragment already included in all 17 review-UI templates — rather than the per-template scattered links it used to take to reach the same destinations (each template's own header used to duplicate a different subset of "Browse questions"/"Jobs"/"Club Management"/etc. links, and a standalone "Messages" button existed on only 6 of the 17). Clicking ☰ opens a `.dropdown-panel` variant (`.nav-panel`, `common_ui.py`) listing every destination the current role can reach: **test bank** (`event_index`), **question bank** (`browse_page`), and **primary sources** (`sources_page`) are each an in-place accordion over `nav_events` (the `_inject_nav()` context processor's per-request accessible-events list — same access rule `index()` uses) since they're per-event; **Jobs** is a direct link to `/admin/jobs` for a coach, or the same per-event accordion pattern for a volunteer (who has no single cross-event jobs view to land on); **Club Management** and **Test management** are coach/volunteer-gated direct links; **Scores** is open to every role; **Notifications** (renamed from the old standalone "Messages" button) closes the panel and calls the pre-existing `toggleToastHistory()`. A student's `nav_events` is always empty (their `events` tuple is never populated), so the three per-event accordions and the volunteer-Jobs branch all correctly disappear for them with no extra role-checking beyond the existing `nav_events`/role conditions already in the template.
...
- `@coach_required` gates routes that aren't event-scoped: event create/edit/delete (`/api/events*`), shared-textbook write routes (`/api/textbooks/upload`, `/<id>/detect`, `/<id>/chapters` — reads stay open to volunteers), and the `/admin/users*` routes below. `GET /settings` itself is **not** coach-gated (every logged-in user has an account to manage) — the template conditionally renders the Manage Users section only when `current_user.role == "coach"`, and the view function only queries `auth.load_users()`/`EVENTS.keys()` for coaches in the first place (a volunteer's render context never holds the full user list at all, not just hides it in markup).
