...
4. **Review** (`review_app.py`) — a browser UI shows each PDF page next to its extracted questions. You can:
   ...
   - **T/F toggle** (per-question card) — marks a question as True/False (`qtype: "tf"`), clearing any choices and swapping the answer field for a True/False dropdown. The extraction pipeline also auto-detects True/False items on its own (a "True or false:"/"T/F:" cue in the stem, or lettered "A. True B. False" choices) — this toggle is for fixing a mis-detected one or converting one by hand. Most Sci-Oly True/False items print no lettered options at all, so this is a distinct question type rather than a synthesized 2-choice MCQ (see spec.md).
