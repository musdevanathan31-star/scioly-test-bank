...
4. **Extract** (`review_app.py`) — a browser UI shows each PDF page next to its extracted questions. You can:
   ...
   - **Question-type control** (a labelled `MCQ · T/F · FRQ · Matching` segmented control, first thing in every card's body) — makes type an explicitly settable property instead of something only inferred from `choices` or buried in a small toggle. Picking **T/F** clears any choices and swaps the answer field for a True/False dropdown; the extraction pipeline also auto-detects True/False items on its own (a "True or false:"/"T/F:" cue in the stem, or lettered "A. True B. False" choices) — the control is for fixing a mis-detected one or converting one by hand. Most Sci-Oly True/False items print no lettered options at all, so this is a distinct question type rather than a synthesized 2-choice MCQ (see spec.md). Picking **Matching** switches to the same two-column editor the drag-capture flow produces, empty and ready to fill in by hand (**+** per column) or by drag-capture (**📋** per column). Every switch is undo-able (Ctrl+Z) since some clear existing data.
