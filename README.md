...
> **Desktop browsers only.** The web UI is built for desktop-sized viewports (≥ 1200 px). Mobile and tablet layouts are explicitly out of scope; the extract page assumes the PDF-pane / question-pane split, drag-to-capture uses a mouse, and the browse page sidebar is always visible. Use Chrome / Edge / Firefox / Safari on a real computer.
...
| `templates/*.html` | Jinja2 page templates for the review UI (`events`, `event_index`, `browse`, `extract`, `sources`, `quiz`, `event_jobs`, `admin_jobs`, `event_scan`, `settings`) |
...
3. **Browse** (`/event/<slug>/browse`) — an event-wide question explorer that aggregates every question across every bucket (PDF-extracted, LLM-generated, scio.ly-scraped) into one searchable, filterable view. Filter by topic, focus, source, **bucket**, validation status, question type (MCQ / FRQ / **Matching** / **True-False**), has-image, **difficulty** (Easy/Medium/Hard/Very Hard/Unrated); sort by Q#, topic, source, length, validation, or difficulty (easiest/hardest first — unrated always sorts last, regardless of direction). Filter state is persisted in the URL, so reload and back-button work and links are shareable. Search box is hotkeyed to `/`; image lightbox on click; sidebar shows live counts of topics/sources/validation.
   ...
   **Validation, both AI and manual** — an **"🤖 AI Validate"** button calls the same Haiku check used elsewhere and persists the verdict immediately (Browse previously couldn't persist this at all). A **Validation** dropdown next to it (Correct/Incorrect/Uncertain/unset) lets a human set or override the status directly — whichever happens most recently simply wins, so a human can always correct a stale or wrong AI verdict, and re-running AI Validate can likewise override a human's. The badge shows who set it (`(ai)`/`(human)`).
   ...
   PDF-sourced questions keep a persistent **"Open source ↗"** link to jump to that PDF's extract page with the question pre-focused (absent for LLM-generated/scio.ly-imported questions, which have no source PDF).

4. **Extract** (`review_app.py`) — a browser UI shows each PDF page next to its extracted questions. You can:
   ...
   - **Drag a rectangle on the PDF** and use the capture button next to the field it targets (stem/choices/answer) to pull that text directly from the source
   ...
   - **Question-type control** (a labelled `MCQ · T/F · FRQ · NUM · Matching` segmented control, first thing in every card's body) — makes type an explicitly settable property instead of something only inferred from `choices` or buried in a small toggle. Picking **T/F** clears any choices and swaps the answer field for a True/False dropdown; the extraction pipeline also auto-detects True/False items on its own (a "True or false:"/"T/F:" cue in the stem, or lettered "A. True B. False" choices) — the control is for fixing a mis-detected one or converting one by hand. Most Sci-Oly True/False items print no lettered options at all, so this is a distinct question type rather than a synthesized 2-choice MCQ (see spec.md). Picking **NUM** makes it a [numerical question](#numerical-questions-value--unit) (value + unit, graded with unit conversion); an FRQ answer like `4.2 m/s` pre-fills it. Picking **Matching** switches to the same two-column editor the drag-capture flow produces, empty and ready to fill in by hand (**+** per column) or by drag-capture (**📋** per column). Every switch is undo-able (Ctrl+Z) since some clear existing data.
   - **MCQ answer by ticking, not typing** — an MCQ's choices render as a two-column table (choice text, correct checkbox); ticking/unticking a row rebuilds `answer` as the comma-joined letters of every ticked choice, in choice order (`"B"`, or `"A, D, E"` when more than one is correct — real Sci-Oly MCQs sometimes have more than one right answer). A free-text answer field stays underneath as the escape hatch for the ~2 dozen questions whose answer is prose rather than a letter (a numeric value with units, etc.) — when `answer` doesn't parse as letters, the checkboxes stay unticked and the text is left alone. Multi-answer MCQs are graded **all-or-nothing** (every correct letter picked, no wrong ones), unlike matching's partial credit — see spec.md.
...
5. **Generate** (per-event `/event/<slug>/sources` page) — pull in third-party questions and feed the LLM new material so it can write fresh questions:
   ...
   ### Numerical questions (value + unit)
   - A numerical question's answer is a **value**, a **unit** and the **quantity** it measures (length, velocity, charge, force, resistance, fraction, … — 32 named quantities, plus "Other (any unit)"), with a **significant-figures** count. Set on the Extract page (**NUM** type) or edited in Browse; the quantity is filled in from the unit, and sig figs from the value as typed (`4.20` → 3).
   - **Grading converts units automatically.** A student may answer in any unit of the right quantity — `15.12 km/h` or `420 cm/s` for a `4.20 m/s` key, `4700 nF` for `4.7 µF`, `298.15 K` for `25 °C`. The converted value is correct when it's within half a unit of the key's last significant figure (`4.20 m/s` with 3 s.f. accepts 4.195–4.205 m/s); change the sig-figs count to widen or narrow that. A unit of the wrong kind (seconds for a speed) scores 0. **The right number with no unit earns half credit**, except for dimensionless/fraction answers, where a bare number is the whole answer (`75 %` accepts `75` or `0.75`).
   ...
   - Trailing zeros in a whole number (`10`, `1200`) are *not* significant under the standard rule, so a `10 ms` key defaults to 1 s.f. and accepts 5–15 ms. Check the s.f. field on such keys.
   ...
   - Sources page → **Import a question bundle** → pick the file → **Upload & preview**. The page is locked with an upload progress bar while the file is sent, and the browser asks before you leave mid-upload. Then a preview shows, **without changing anything yet**: counts by topic × type, topics that will be remapped, duplicates of questions already in the bank, missing or refused images, anything that can't be imported, and a warning if the bundle names a different event.
   - **Import** runs as a background job, so you can leave the page. Coming back to the Sources page picks the import back up: live progress while it runs, or the result once it's done (kept until you click **Dismiss**). It's also on the event's **Jobs** page and in the header's `⏳ N jobs` badge. A cancelled or failed import adds nothing.
   - **Mark all as validated** works like the JSON import's, including the gradeability check: a question no grader could mark right is imported but left unvalidated, and listed.
   - How fields land: `numerical` questions become real [numerical questions](#numerical-questions-value--unit) when the answer splits into value + unit (a bundle may also give `unit`/`quantity`/`sig_figs` explicitly); one that doesn't parse is imported as free response, marked `import_meta.qtype = "numerical"`. Topics not in the event's list are remapped (case-insensitive name match, else the keyword classifier, which is given the bundle's topic name as a hint). The original topic name is kept in `import_meta.topic`. Images are saved under server-chosen names (`imp_<bundle-hash>_<name>`); only PNG/JPEG/GIF/WebP whose bytes match their extension are accepted (no SVG), up to 10 MB each and 200 MB per bundle.
   - Questions go into the same `_generated_<event>.pdf` bucket as generated/JSON-imported ones, with the same whole-bank duplicate check — re-importing a bundle just reports everything as a duplicate.
   ...
   - Any field outside this list (e.g. a difficulty rating) is silently ignored on import, not stored — if you want difficulty tracked, fold it into `rationale` or `source_snippet` as free text instead of inventing a new top-level key, or use a [question bundle](#import-a-question-bundle-zip--questions--images--difficulty) instead, which keeps difficulty and images.
...
- `annotations[<pdf>]` — your edits, organised by what you did:
  ...
  - `contexts` — shared context blocks (a passage, table, or diagram that several questions reference — e.g. a Disease Detectives case study, or a Circuit Lab resistor-network diagram with two or three sub-questions): `[{id, title?, text, images:[], pages:[]}]`. A question opts in via `context_id`; the extract UI then renders the context above the stem and edits to the shared text flow through to every linked question. Contexts can span pages just like questions. `title` is an optional human label (e.g. "Avian flu outbreak") shown wherever a context is picked from a list — falls back to the auto id (`ctx_1`, …) when blank. A context's `images` list works the same way a question's does: each context card has a **"📌 Add figure"** button — drag a rectangle on the PDF around the diagram/figure and it's cropped, saved, and appended to `images` (the region is saved as a PNG, never OCR'd as text); the same `×` remover on each thumbnail splices it back out (the PNG itself is left on disk, same as for question images).

    **Grouping several questions at once**: linking one question at a time via its Context dropdown doesn't scale to a 6-question group. Every question row on the extract page carries a permanent selection checkbox — there's no mode to turn on first; ticking any one of them (or using **Tools ▾ → "🔗 Group questions on this page"** to tick every question on the current page at once) is what activates the selection, and a contextual bar appears right under the toolbar with the picked-question count, **"Assign to context ▾"**, and **Clear**. The selection persists across page turns (a group's shared context often spans pages), and once anything is ticked, clicking a question's bounding box on the PDF pane toggles it too. **"Assign to context ▾"** bulk-links the whole selection to an existing context or to a freshly captured one ("+ New context from region" auto-links the capture to whatever's currently selected); clearing the selection makes the bar disappear again. Works either direction — capture the passage/diagram first then select its questions, or select the questions first then capture the shared material. A collapsible **"📎 Question groups in this PDF"** panel above the page-specific cards lists every context regardless of which page you're on, with linked-question jump links and inline title editing — the per-page section only shows a context once you're on a page it touches, so this is the place to audit a multi-page group's full membership.

    **Where grouping shows up beyond the extract page**: the quiz page's "Keep question groups together" checkbox (off by default, so today's behavior is unchanged unless you opt in) expands the selection to pull in every sibling of a picked grouped question and shows the shared context above each one; `build_markdown()`/the PDF/CSV/JSON exports cluster a group's questions together with the shared context rendered once, instead of scattering them across the topic listing. **Published assessments are stricter, and unconditionally so**: `assessments.start_or_get_response()` shuffles whole groups rather than individual questions, so a-then-b sub-questions are never served out of order or with the shared diagram banner repeated at unrelated points in the test — see spec.md's testing section for why this is a correctness rule, not an opt-in preference like the quiz page's checkbox. The student take page also collapses a repeated context banner into a one-line "Refer to the diagram above" affordance when the previous question in the student's order shared the same context, so the figure doesn't re-consume the screen on every sub-question.

    **Fixing a mis-parsed question group ("⤵ Split into group")**: a shared-context question set that labels its sub-questions `a.` `b.` `c.` is indistinguishable from MCQ choice markers, so `split_choices()` swallows the sub-questions as lettered options of one multiple-choice question instead of recognizing them as separate, separately-graded sub-questions — confirmed against real extracted data (a Circuit Lab question whose "choices" were three full sub-questions sharing one figure, `answer` empty, one "choice" even carrying its own `(1 point)` marker, which a real choice never does). This is a **manual, per-question, undo-able** action, never automatic detection — about a third of the questions with this exact shape (2+ choices sharing a figure) are genuine MCQs with long declarative choices, and a heuristic would destroy them. On an MCQ with 2+ choices, its **Content ▸ Type row** — the same MCQ/T-F/FRQ/Matching segmented control every card starts with — grows a 5th button, **"⤵ Split into group"** (disabled with an explanatory tooltip below 2 choices, and absent entirely for a non-MCQ type); clicking it (after a confirm) takes an undo snapshot, then resolves a context (reuses `context_id` if already linked, otherwise creates one shaped exactly like "+ Add context from region"'s output — `text` = the question's stem, `images` = the question's images since the figure is shared by the whole group not owned by one sub-question, `pages` = the question's page), turns each choice into its own FRQ (`qtype:"frq"`, `choices:[]`, trailing `(N points)` markers stripped) linked to that context, and deletes the original. Numbering follows the extractor's own convention verified against the corpus (136 `Nb`, 53 `Nc`, 28 `Nd`, down to `Nh`): the first sub-question keeps the original bare number, later ones get letter suffixes — `21`, `21b`, `21c`, … never `21a` — skipping any suffix already used elsewhere in the bank. A pre-existing non-empty `answer` on the original (unusual for this shape) is never silently dropped — it's appended into the new context's text as `(Original answer: …)` and named in the status message.

    **Capturing a multi-part question directly ("🧩 Add multi-part question from region", Add ▾)**: building a group from a fresh PDF region used to take two disconnected steps — capture it as a plain question (which, per the ambiguity above, OCR's it as one MCQ), then separately find and click "⤵ Split into group". This Add ▾ button does both in one drag: the same region-OCR call "+ Add question from region" already makes, but interpreted directly as a group when it finds 2+ lettered parts (same construction as "⤵ Split into group" above, skipping the intermediate MCQ-shaped question entirely) — or as a single plain question, with an explanatory status message, when it doesn't.

Writes are atomic (tempfile + `os.replace`) and serialised by a per-event lock, so concurrent saves never leave a half-written JSON on disk. The lock spans the whole load → modify → save cycle for every short request-handler save (annotation edits, Browse-page PATCHes, imports), not just the write itself — so two coaches editing the same event around the same moment can't silently lose one edit to the other (the same guarantee applies to `auth.py`'s users file, `seasons.py`'s seasons/rosters, and `assessments.py`'s windows/assessments/responses files). The one deliberate exception is a long-running OCR/extraction job's own checkpoint saves, which hold a single in-memory snapshot for the job's full duration rather than re-locking per checkpoint — concurrent edits to a *different* PDF in the same event during that job are unaffected, but an edit to the *same* PDF mid-job can still be overwritten by the job's next checkpoint.
...
The extract page's **Reprocess ▾** menu offers three modes:
...
The review UI renders PDF pages to PNG on demand. That is real CPU on the single gunicorn worker students' answer-saves also share — measured on a real Circuit Lab PDF: 26.5ms per page at 120dpi, 72ms at 200dpi — and the extract workflow re-renders the same page constantly while zooming, capturing regions and paging back and forth.
...
Two export formats need a package that isn't in `requirements.txt`, because neither is needed to run the app: **Anki deck** (`genanki`) and **Printable PDF** (`reportlab`). `reportlab` also drives the Assessments dashboard's **Test** and **Key** downloads, which produce a PDF **with question figures embedded** when it's installed and fall back to markdown when it isn't — markdown can't carry an image, so a question about a labelled diagram exports as a bare filename reference and is useless on paper. That fallback means the buttons always produce something; it just produces a worse document. Printable PDF works with any export scope — everything, the filtered set, or just the questions you've ticked — and offers the same three answer layouts as the markdown export: **questions only**, **answers inline**, and **key at end**. The filename records which one you picked, since three layouts landing on one name is how the copy with the answers gets handed out by mistake. Anki decks are still whole-bank only, and the menu disables that option rather than silently ignoring a narrower scope.
...
- **Student** — no question-bank access at all (not even read-only — see `spec.md` §17), with one deliberately narrow exception: figures referenced by an assessment they're currently taking or whose results were released to them, served through `/my-assessments/<id>/image/<fname>` against an allowlist built from that assessment's own frozen snapshot (so a diagram attached to a grouped question set actually renders, without exposing the rest of the event's `images/`). Scoped entirely to the assessments they're rostered on for the current season: `/my-assessments` (take a live assessment, view released results) and `/scores` (**their own scores only**, as of 2026-08-26 — see `spec.md` §11; response-level detail remains restricted to coaches and whoever actually graded that assessment).
...
See [MIGRATION.md's "Deployment values template"](MIGRATION.md#deployment-values-template) for an example structure you can adapt to your environment.
...
See [MIGRATION.md's "Deployment values template"](MIGRATION.md#deployment-values-template) for placeholders you can fill in per instance. Per instance, two complementary mechanisms, neither touching `.env`/`auth_users.json` (handle those separately — see below):
...
[`deploy/provision-host.sh`](deploy/provision-host.sh) turns a bare RHEL box into a server that can run this app. Everything above in "Deploying" and "Running multiple independent instances", plus MIGRATION.md's "Deployment values template", describes the same bring-up in prose; this script *is* that prose, executable and idempotent:
...
- **Reprocess → "wipe annotations" / "manual mode"** snapshots the PDF's annotations, manual-edit tracking, and question list to `<event>/.archive/<pdfname>/<timestamp>.json` (`archive.py`) *before* wiping anything. Restore any snapshot from the Reprocess dropdown's "Snapshot history" entry on the extract page.
...
The extract page's empty-PDFs state lets you trigger `download_event.py` from the browser with a live progress bar — no terminal required.
