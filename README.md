...
- [Deployment values template](#deployment-values-template)
  - [Admin app](#admin-app)
...
See [Deployment values template](#deployment-values-template) below for an example structure you can adapt to your environment.
...
See [Deployment values template](#deployment-values-template) for placeholders you can fill in per instance. Per instance, two complementary mechanisms, neither touching `.env`/`auth_users.json` (handle those separately — see below):
...
[`deploy/provision-host.sh`](deploy/provision-host.sh) turns a bare RHEL box into a server that can run this app. Everything above in "Deploying", "Running multiple independent instances" and "Deployment values template" describes the same bring-up in prose; this script *is* that prose, executable and idempotent:
